...
function buildPrompt() {
  ...
  const cutoffJst = new Date(nowJst.getTime() - 24 * 3600000); // 24時間前(JST)
...
【検索対象（期間）】
${cutoffStr} 〜 ${nowStr}（日本時間／直近24時間）に公表・報道された情報の中から、日本国内のホテル・リゾートマーケットに影響を与える重要ニュースを検索してください。以下の観点を重視してください。
...
【出力条件】
- 必ず各記事の公開日時を確認し、${cutoffStr} 以降（直近24時間以内）に公表された情報だけを対象にする
- 重要度の高いニュースを最大5件
- 日本語で要約
